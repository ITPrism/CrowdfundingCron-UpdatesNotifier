...
class plgCrowdfundingCronUpdatesNotifier extends JPlugin
{
    ...
    protected function getUpdates(array $options = array())
    {
        $db     = JFactory::getDbo();
        $query  = $db->getQuery(true);

        // Truncate string.
        $descriptionLength = (int)abs(Joomla\Utilities\ArrayHelper::getValue($options, 'description_length', 0, 'int'));
        ...
        if (count($updates) > 0) {
            ...
            foreach ($updates as $update) {
                ...
                $results[$update['user_id']]['name']  = ($user !== null) ? $user->getName() : '';
                $results[$update['user_id']]['email'] = ($user !== null) ? $user->getEmail() : '';

                $project = $projects->getProject($update['project_id']);

                $results[$update['user_id']]['projects'][$update['project_id']]['title'] = ($project !== null) ? $project->getTitle() : '';
                $results[$update['user_id']]['projects'][$update['project_id']]['slug'] = ($project !== null) ? $project->getSlug() : '';
                $results[$update['user_id']]['projects'][$update['project_id']]['catslug'] = ($project !== null) ? $project->getCatSlug() : '';
